...
export const getReadPercent = (pagesRead, totalPages) => {
  ...
  return Number((pagesRead / totalPages) * 100).toFixed(1);
};
...
export const getReadingMinutes = (start, finish) =>
  Math.round((new Date(finish) - new Date(start)) / 24 / 60);

// export const getReadingMinutes = (start, finish) =>
//   Number((new Date(finish) - new Date(start)) / 60);
...
export const getTotalMinutes = progress => {
  ...
  return progress.reduce((sum, { startReading, finishReading }) => {
