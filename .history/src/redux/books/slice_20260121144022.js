  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(fetchMyLibrary.fulfilled, (state, action) => {
        ...
        state.myLibrary.items = action.payload.map(book => ({
          ...book,
          status: book.progress?.some(p => !p.finishReading)
            ? 'in-progress'
            : book.status || 'unread',
        }));
      })
      ...
      .addCase(myBookReadingStart.fulfilled, (state, action) => {
        console.log('START RESPONSE:', action.payload);
        ...
        const book = state.myLibrary.items.find(
