...
import { selectFilterStatus } from '../filters/selectors.js';
...
export const selectFilteredMyLibrary = createSelector(
  [state => state.books.myLibrary.items, selectFilterStatus],
  (books, filterStatus) => {
    if (filterStatus === 'all') return books;

    return books.filter(book => getBookStatus(book) === filterStatus);
  }
);
