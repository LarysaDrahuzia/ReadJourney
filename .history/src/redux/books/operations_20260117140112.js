...
export const fetchBookById = createAsyncThunk(
  ...
  async (_id, thunkAPI) => {
    try {
      const response = await api.get(`/books/${_id}`);
...
export const myBookReadingStart = createAsyncThunk(
  ...
  async ({ bookId, startPage }, thunkAPI) => {
    try {
      const { data } = await axios.post(`/books/reading/start/${bookId}`, {
        startPage,
      });
      return data;
    } catch (error) {
      return thunkAPI.rejectWithValue(
        error.response?.data?.message || 'Failed to start reading the book'
      );
    }
  }
);
...
export const myBookReadingFinish = createAsyncThunk(
  ...
  async ({ bookId, endPage }, thunkAPI) => {
    try {
      const { data } = await axios.post(`/books/reading/finish/${bookId}`, {
        endPage,
      });
      return data;
    } catch (error) {
      return thunkAPI.rejectWithValue(
        error.response?.data?.message || 'Failed to finish reading the book'
      );
    }
  }
);
...
export const deleteReading = createAsyncThunk(
  ...
  async ({ bookId, readingId }, thunkAPI) => {
    try {
      const { data } = await axios.delete(`/books/reading`, {
