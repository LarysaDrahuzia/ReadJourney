...
export const myBookReadingFinish = createAsyncThunk(
  ...
  async ({ bookId, endPage }, thunkAPI) => {
    try {
      ...
      const progress = state.books.reading.progress;
      const activeReading = progress.find(item => !item.finishReading);

      if (!activeReading) {
        return thunkAPI.rejectWithValue('No active reading session');
      }

      const { data } = await api.post('/books/reading/finish', {
        id: bookId,
        readingId: activeReading._id,
        page: endPage,
      });
      return data;
    } catch (error) {
